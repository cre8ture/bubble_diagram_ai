...
export const POST = (async ({ request }) => {
  ...
  if (prompt.startsWith("make_outline_320_")) {
    ...
    // Handle accordingly
    directiveToApi = `generate a JSON data structure that conceptualizes the key elements related to ${prompt}. The structure should represent relationships between the key elements in the format:
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
            {
              role: 'user',
              content: `generate a JSON data structure that conceptualizes the key elements related to ${prompt}. The structure should represent relationships between the key elements in the format:
