<script>
  ...
  function updateViewBox(svg) {
    ...
    var minX =
        dimensions.minX /* minimum x-coordinate of your graph elements */,
...
let nodeContainer = svg.append("g").attr("class", "node-container");
    ...
    // Update links using allLinks instead of newLinksArray
    // let link = svg.selectAll(".link")
    //     .data(allLinks, d => d.source.name + "-" + d.target.name);
    let link = linkContainer.selectAll(".link")
    .data(allLinks, d => d.source.name + "-" + d.target.name);
    ...
    simulation.force("link").links(allLinks).id(d => d.name);
...
//   function update3(newNodes, newLinks) {
//     svg.selectAll(".node");
//     link = svg.selectAll(".link");

//     simulation
//     .nodes(Object.values(newNodes))
//       .force("link", d3.forceLink(newLinks).distance(100)) // Decreased link distance
...
//     console.log("newNodes, newLinks", newNodes, newLinks)

//     // Join new data
//     const nodeEnter = node.data(newNodes, (d) => d.name).enter();
//     const nodeUpdate = node.data(newNodes, (d) => d.name);

//     console.log(nodeEnter, nodeUpdate)
//     const linkEnter = link
//       .data(newLinks, (d) => d.source.name + "-" + d.target.name)
//       .enter();

//     const linkUpdate = link.data(
//       newLinks,
...
//     // simulation.alpha(1).restart();
//     // Start simulation
// simulation.on("tick", tick) // Set tick handler
//           .restart()
...
//   }
  ...
  function update2() {
    ...
    force.alphaTarget(1).on("tick", tick).restart();
  ...
  $: {
    if (new_bubble_graph) {
      // count = 0
      console.log("new_bubble_graph", new_bubble_graph);
  ...
  $: {
    if (new_node_to_add) {
      // svg.selectAll("*").remove();
      // build_bubble_graph(new_node_to_add);
      console.log("new_node_to_add", new_node_to_add);
